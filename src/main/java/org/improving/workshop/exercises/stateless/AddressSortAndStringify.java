...
public class AddressSortAndStringify {
    ...
    static void configureTopology(final StreamsBuilder builder) {
        builder
            ...
            // 'MN'
            .branch(
                    (state, address) -> "MN".equals(state),
                    Branched.withConsumer(ks -> ks.to(MN_OUTPUT_TOPIC, Produced.with(Serdes.String(), Serdes.String())))
            )
            .defaultBranch(
                    Branched.withConsumer(ks -> ks.to(DEFAULT_OUTPUT_TOPIC, Produced.with(Serdes.String(), Serdes.String())))
            );
    }
}